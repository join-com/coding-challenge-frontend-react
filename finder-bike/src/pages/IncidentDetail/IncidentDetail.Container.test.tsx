...
import { incidentDetailReducer } from './IncidentDetail.reducer';
...
function renderWithRedux(
  ui,
  {
    initialState = { incidentDetailReducer: {} },
    store = createStore(incidentDetailReducer, initialState)
  } = {}
) {
  return {
...
test('can render with redux with defaults', () => {
  const { getByText } = renderWithRedux(<IncidentDetailContainer />);
  expect(getByText('POLICE DEPARTMENT')).toBeTruthy();
});

test('can render with redux with data with incident', async () => {
